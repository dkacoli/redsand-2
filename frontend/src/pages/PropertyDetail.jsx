...
import { 
  MapPin, Bed, Bath, Maximize, TrendingUp, ArrowLeft, 
  Check, Building2, Calendar, Share2
} from "lucide-react";
  ...
  if (!property) {
    return (
      <div className="min-h-screen pt-32 flex flex-col items-center justify-center">
        ...
        <p className="text-white/50 mb-8">The property you're looking for doesn't exist or has been removed.</p>
  ...
  return (
    <div data-testid="property-detail-page" className="min-h-screen pt-24 md:pt-32">
      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 mb-8">
        <Link
          to={property.property_type === "investment" ? "/investment" : "/residential"}
          data-testid="back-to-listings"
          className="inline-flex items-center gap-2 text-gold hover:text-gold-hover transition-colors text-sm"
        >
          <ArrowLeft size={16} />
          Back to {property.property_type === "investment" ? "Investment" : "Residential"} Properties
        </Link>
      </div>
          ...
          <div className="grid grid-cols-3 lg:grid-cols-1 gap-4">
            {images.slice(0, 3).map((img, index) => (
              <button
                ...
                className={`aspect-[16/10] overflow-hidden border-2 transition-all ${
                  activeImage === index ? "border-gold" : "border-transparent hover:border-gold/50"
                }`}
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="mb-8 p-6 glass">
              ...
              <p className="font-playfair text-4xl md:text-5xl text-white">
                {formatPrice(property.price)}
              </p>
              {property.property_type === "investment" && property.rental_yield && (
                <p className="text-white/60 mt-2">
                  Estimated Rental Yield: <span className="text-gold">{property.rental_yield}%</span>
                </p>
              )}
            </div>
            ...
            {property.features?.length > 0 && (
              <div>
                <h3 className="font-playfair text-2xl text-white mb-4">Features & Amenities</h3>
          ...
          <div className="lg:col-span-1">
            <div className="glass p-8 sticky top-32">
              ...
              <Link
                to="/contact"
                data-testid="contact-for-property"
